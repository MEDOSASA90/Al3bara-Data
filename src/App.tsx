...
function viewFromHash(): ViewMode {
  ...
  const known: ViewMode[] = ['dashboard', 'entities', 'advances', 'work', 'partnerships', 'brochures', 'archiveMenu', 'archiveEntities', 'archiveWork', 'archiveAdvances'];
...
export default function App(): ReactNode {
  ...
  // FCM: سجل توكن الجهاز أول ما المستخدم يسجل دخول (اشعار فوري حتى والتطبيق مقفول)
  useEffect(() => {
    if (!user?.uid) return;
    void (async () => {
      const { registerPushToken } = await import('./services/notifications');

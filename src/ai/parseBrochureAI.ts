...
export async function brochureTextToData(rawText: string, auctionDateHint?: string): Promise<BrochureAIResult> {
  ...
  const countLine =
    lotCountEstimate > 0
      ...
      : '';
  const isChunk = text.includes('--- PAGE') && text.match(/--- PAGE \d+ ---/g) !== null && (text.match(/--- PAGE \d+ ---/g) ?? []).length < 25;
  const chunkLine = isChunk
    ? `مهم: النص ده جزء من كراسة مزاد كبيرة (صفحات متتابعة). حلل كل صفحة جواه بالكامل — كل جهة وكل لوط مكتوب في الصفحات دي لازم يظهر في النتيجة. الجزيئات التانية هتتحلل على حدة.\n`
    : '';
  const prompt =
    `${hintLine}${countLine}${chunkLine}` +
    ...
    `5. سطور الفهرس (جدول فيه أرقام لوطات وأعداد بنود) تجاهلها — استخرج اللوطات من صفحات التفاصيل فقط. لكن إذا النص كله فهرس أعد جهاته كـ entities بأسمائها فقط.\n\n` +
    `نص الكراسة:\n${text}`;
  return generateJson({
    system: BROCHURE_SYSTEM,
    prompt,
